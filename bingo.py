...
#If you spot any bugs, you can email me!
...
def board(allgoals):
  ...
  finds = bool(input("Use only finds? "))
  ubans = bool(input("Any limitations? "))
  if ubans:
    ubans = " "
    while not set(ubans).issubset(set("PTRS/G£OMIENC1.L?*")):
      ubans = input(
...
Exclude anything you can't do at night (N)
Exclude any goals involving colours (C)
Exclude any goals involving opponents (1)
Exclude any goals involving specific locations (.)
Exclude any goals involving language (L)
Exclude any goals requiring other goals (?)
All of the above (*)
(Enter nothing to cancel)
""").upper()
    if ubans == "*":
      ubans = "PTRS/G£OMIENC1.L?"
  ...
  while len(codegoals) < 25:
    ...
    goal = all[r]
    if not (str(r + 1),
            goal) in codegoals and not (nobans and "Don't" in goal) and not (
                finds and goal[:4] != "Find"
            ):  #No repeats or bans if disallowed and only finds if specified
      types = []
      if ubans != "" and "(" in goal:
        types = goal[goal.find("("):].strip("()").split(",")
      if types == [] or ubans == "" or set(types).intersection(
          set(ubans)) == set():  #Not universally banned
    ...
    if not importing:
      while "[" in goal:
        if goal[goal.find("[") + 1].isdigit():
          x = random.randint(
              int(goal[goal.find("[") + 1:goal.find("-")]),
              int(goal[goal.find("-") +
                       1:goal.find("]")]))  #Pick a random number in the range
          if x == 1:
            goal = goal.strip().rstrip("s")
          goal = goal[:goal.find("[")] + str(x) + goal[goal.find("]") +
                                                       1:]  #Insert x
        else:
          x = chr(
              random.randint(ord(goal[goal.find("[") + 1]),
                             ord(goal[goal.find("-") +
                                      1])))  #Pick a random letter in the range
          goal = goal[:goal.find("[")] + x + goal[goal.find("]") +
                                                  1:]  #Insert x
        if code != []:
          code[i] += "." + str(x)
    else:
...
print("\033C")
print(
    """Use the following commands. The ones in brackets require a board to be generated or imported first.
